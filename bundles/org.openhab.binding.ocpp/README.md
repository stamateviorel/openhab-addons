...
The OCPP profile-1 rule is 16–20 visible ASCII characters for 1.6 and 16–40 for 2.0.1, but the binding does not enforce either window: it accepts whatever the charger presents when no `authPassword` is set, and compares the password exactly when one is. The library would otherwise refuse a handshake on length alone, before the binding saw it, which silently locked out chargers that always send a header with a short or empty password.
